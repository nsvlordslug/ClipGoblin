...
fn run_analysis_signals(
    ...
    for (i, c) in selected.iter().enumerate() {
        ...
        // Phase A (amended) safety net: re-apply the broader single-signal-
        // with-shock-tag cap at persist time. Mirrors the predicate in
        // score_clip_candidate. kw_boost is added AFTER score_clip_candidate
        // ran, which would otherwise push a capped 0.65 total back up by as
        // much as +0.20. The persisted virality_score is what the Review UI /
        // Clips page display, so this guarantees the user-facing "70%+ feels
        // boring" complaint is addressed end-to-end.
        // See docs/superpowers/specs/2026-05-07-phase-a-scoring-fix-design.md §7a
        let has_shock_family_tag_at_persist = {
