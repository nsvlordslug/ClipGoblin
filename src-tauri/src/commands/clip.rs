...
fn caption_words(srt: &str) -> Vec<String> {
    srt.lines()
        .filter(|line| {
            let trimmed = line.trim();
            !trimmed.is_empty() && !trimmed.contains("-->") && trimmed.parse::<u32>().is_err()
        })
        ...
        .filter_map(|word| {
            let cleaned = word
                ...
                .trim();
            (!cleaned.is_empty() && cleaned.chars().count() <= 40).then(|| cleaned.to_string())
        })
        .take(1_200)
        .collect()
}

fn repeated_correction_candidates(before: &str, after: &str) -> Vec<(String, String)> {
    let before_words = caption_words(before);
    let after_words = caption_words(after);
    ...
    before_words
        .into_iter()
        .zip(after_words)
        .filter_map(|(source, corrected)| {
            let source_key = source.to_lowercase();
            let corrected_key = corrected.to_lowercase();
            if source_key == corrected_key
                || source_key
                    .chars()
...
fn supported_branding_extension(path: &std::path::Path) -> Option<String> {
    ...
    matches!(extension.as_str(), "png" | "jpg" | "jpeg" | "webp" | "gif").then_some(extension)
}
...
pub fn update_clip_settings(
    ...
    game: Option<String>,
    captions_provenance: Option<String>,
    caption_audio_mode: Option<String>,
    db: State<'_, DbConn>,
) -> Result<(), String> {
    ...
    let captions_text = captions_text.filter(|value| !value.trim().is_empty());
    let captions_changed =
        before.captions_text.as_deref().map(str::trim) != captions_text.as_deref().map(str::trim);
    let learned_corrections = if db::transcription_correction_learning_enabled(&conn)
        && captions_changed
        && matches!(
            db::normalize_captions_provenance(&before.captions_provenance),
            "aligned" | "analysis-draft"
        ) {
        repeated_correction_candidates(
            before.captions_text.as_deref().unwrap_or(""),
            captions_text.as_deref().unwrap_or(""),
        )
    } else {
        Vec::new()
    };
    ...
    let mut captions_provenance = captions_provenance
        .as_deref()
        .map(db::normalize_captions_provenance)
        .unwrap_or_else(|| db::normalize_captions_provenance(&before.captions_provenance))
        .to_string();
    if captions_text.is_none() {
        captions_enabled = 0;
        captions_provenance = "none".to_string();
        captions_pipeline_version = 0;
    } else if captions_changed {
        captions_provenance = "edited".to_string();
        captions_pipeline_version = 0;
    } else if boundaries_changed
        && matches!(captions_provenance.as_str(), "aligned" | "analysis-draft")
    {
        captions_provenance = "analysis-draft".to_string();
        captions_pipeline_version = 0;
    }
    let caption_audio_mode = db::normalize_caption_audio_mode(
        caption_audio_mode
            .as_deref()
            .unwrap_or(&before.caption_audio_mode),
    );
    let recognition_invalidated = captions_changed
        || boundaries_changed
        || caption_audio_mode != db::normalize_caption_audio_mode(&before.caption_audio_mode);
    db::update_clip_settings(
        ...
        context_background_path.as_deref(),
        context_background_mode,
        context_blur_strength,
        context_video_y,
        full_frame_scale,
        game.as_deref(),
        &captions_provenance,
        captions_pipeline_version,
        caption_audio_mode,
    )
    .map_err(|e| format!("DB error: {}", e))?;
    if recognition_invalidated {
        conn.execute(
            "UPDATE clips
                SET captions_recognition_signature = NULL,
                    captions_language = NULL,
                    caption_audio_stream = NULL
              WHERE id = ?1",
            [&clip_id],
        )
        .map_err(|e| format!("DB error clearing stale recognition metadata: {e}"))?;
    }
    if !learned_corrections.is_empty() {
        db::record_transcription_corrections(&conn, &learned_corrections)
            .map_err(|e| format!("DB error saving local vocabulary context: {e}"))?;
    }
        ...
        || (before.end_seconds - end_seconds).abs() >= 0.75
    {
        let dedupe_key = format!(
            "trim:{}:{:.1}:{:.1}:{:.1}:{:.1}",
            clip_id, before.start_seconds, before.end_seconds, start_seconds, end_seconds,
        );
...
pub fn save_clip_review(
    ...
    let approved_bounds = if rating.as_deref() == Some("good") {
        reviewed_clip
            .as_ref()
            .map(|(_, start_seconds, end_seconds)| (*start_seconds, *end_seconds))
    } else {
        None
    };

    db::set_clip_review_with_bounds(
        &mut conn,
        &highlight_id,
        rating.as_deref(),
        note.as_deref(),
    ...
    if let Some((clip_id, _, _)) = reviewed_clip {
        let dedupe_key = format!(
            "review:{}:{}:{}",
            clip_id,
...
#[tauri::command]
pub fn get_personalization_status(db: State<'_, DbConn>) -> Result<PersonalizationStatus, String> {
    let conn = db.lock().map_err(|e| format!("DB lock: {}", e))?;
...
#[tauri::command]
pub fn export_review_data_for_vod(vod_id: String, db: State<'_, DbConn>) -> Result<String, String> {
    let conn = db.lock().map_err(|e| format!("DB lock: {}", e))?;
    ...
    serde_json::to_string_pretty(&payload).map_err(|e| format!("JSON serialization error: {}", e))
}

#[cfg(test)]
mod transcription_correction_tests {
    use super::repeated_correction_candidates;

    #[test]
    fn learns_only_word_for_word_manual_corrections() {
        let before = "1\n00:00:00,000 --> 00:00:01,000\nhello stacy\n";
        let after = "1\n00:00:00,000 --> 00:00:01,000\nhello Stacie\n";
        assert_eq!(
            repeated_correction_candidates(before, after),
            vec![("stacy".to_string(), "Stacie".to_string())]
        );
    }

    #[test]
    fn ignores_rewrites_that_shift_word_alignment() {
        let before = "1\n00:00:00,000 --> 00:00:01,000\nthat worked\n";
        let after = "1\n00:00:00,000 --> 00:00:01,000\nthat really worked\n";
