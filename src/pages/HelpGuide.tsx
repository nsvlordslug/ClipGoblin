      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          {matchesSearch('personalized detection learn ratings good meh boring future local') && <FaqItem q="How does personalized detection learn from me?">
            Turn on Personalized detection feedback in Settings → Detection. Re-analyzing the same VOD keeps clips marked Good, excludes events marked Boring, Wrong moment, or Duplicate, including shifted cuts that still overlap the rejected event, and applies timing corrections. When BYOK detection is enabled, reviewed ranges are reserved before the existing judge call and Good dialogue becomes a taste example for finding new moments. Varied Good, Meh, and Boring choices teach broader ranking for future VODs. The profile stays on this PC.
          </FaqItem>}
          ...
          {matchesSearch('reanalyze vod sensitivity same vod personalization') && <FaqItem q="Can I re-analyze a VOD with different sensitivity or new feedback?">
            Yes — open the VOD card menu and click Re-analyze VOD. The new run uses the current sensitivity and local profile, carries Good clips forward, excludes meaningful overlap with explicitly rejected events, and applies timing feedback before choosing replacements. If feedback leaves fewer than three normal-score results, ClipGoblin can surface the strongest structurally safe alternatives for you to rate; it never restores a rejected event just to fill the list. A successful re-analysis replaces that VOD's other generated clips, while its learning evidence remains.
          </FaqItem>}
          ...
          {matchesSearch('stream style auto action cozy story talking mixed reanalyze preserve ratings timing') && <FaqItem q="What does the VOD stream style change?">
            <b>Auto</b> uses the saved game to show and choose a broad detection style. Action favors faster moments, Cozy and Talking give quieter speech more weight, Story allows more buildup, and Mixed uses balanced defaults. Changing a completed VOD does not run anything automatically: confirm <b>Re-analyze as...</b> when ready. Good clips, ratings, and manual timing stay saved, while other candidates may differ.
          </FaqItem>}
          ...
          <div className="space-y-2">
            {deepDiveOpen === 'getting-started' && (
              <HelpSection key="getting-started" icon="🎯" title="Getting started" defaultOpen>
                ...
                <p><span className="text-white font-medium">3. Check the game and style</span> — Medal uses the game-folder name. For Twitch, click Set game when the historical category is missing or wrong. Leave the compact stream-style selector on Auto unless its displayed guess does not match the stream.</p>
            ...
            {deepDiveOpen === 'personalization' && (
              <HelpSection key="personalization" icon="🎯" title="Teach ClipGoblin your taste" defaultOpen>
                ...
                <p><span className="text-white font-medium">Rate moment quality</span> — On the Clips page, open the checklist icon on a clip and choose Good, Meh, or Boring. Good keeps that moment in same-VOD results; Boring prevents it from returning. Use at least two different ratings to teach broader ranking across other VODs.</p>
                <p><span className="text-white font-medium">Report edit issues</span> — Select every chip that applies: Starts too late, Cuts off early, Too long, Wrong moment, or Duplicate. Wrong moment and Duplicate immediately exclude that window on same-VOD reanalysis. Timing flags force a changed cut, and both timing chips can be selected together.</p>
                <p><span className="text-white font-medium">What changes</span> — Re-analyzing the same VOD preserves Good moments and removes explicitly rejected events and overlapping recuts. BYOK detection removes all reviewed ranges from the candidate transcript, uses Good dialogue as taste examples, and asks the judge for different events in the same call. Local detection can surface up to three structurally safe exploration picks when the normal score floor would otherwise leave almost nothing. Broader personalization gently reorders future candidates toward your learned taste.</p>
            ...
            {deepDiveOpen === 'editor' && (
              <HelpSection key="editor" icon="🎨" title="Editor, captions, and branding" defaultOpen>
                <p><span className="text-white font-medium">Edit tab</span> — Adjust trim, aspect ratio, export preset, and layout. Context Fit is the recommended vertical-gameplay layout because it keeps the entire scene visible over a soft video blur, clean black bars, or branding; Video placement moves that fitted frame up or down. Full Frame is available when you deliberately want a vertical center crop, with a Zoom out slider that can reveal up to 30% more of the source. Landscape / Widescreen creates an upright 16:9 export that preserves the full game and HUD. Split and PiP keep gameplay and a second panel visible together.</p>
