...
12. **TikTok Direct Post was approved 2026-09-15.** Show only the audiences returned by `creator_info`, require an explicit selection, and preserve TikTok's disclosure, duration, consent, and interaction restrictions. If TikTok unexpectedly returns `unaudited_client_can_only_post_to_private_accounts`, ask the user to reconnect and investigate the live app status instead of restoring a private-only gate.
